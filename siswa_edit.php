...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['submit'] )){
		...
		$sql = mysql_query("UPDATE siswa SET nama='$nama', idprodi='$idprodi', idthajaran='$idthajaran' WHERE nis='$nis'");
		...
		list($nis,$nama,$idprodi,$idthajaran) = mysql_fetch_array($sql);
?>
...
<form method="post" action="admin.php?hlm=master&sub=siswa&aksi=edit" class="form-horizontal" role="form">
	...
	<div class="form-group">
		...
		<div class="col-sm-4">
			...
			<?php
			$qtapel = mysql_query("SELECT * FROM tapel ORDER BY id");
			while(list($id,$thajaran)=mysql_fetch_array($qtapel)){
				echo '<option value="'.$id.'"';
				echo ($id==$idthajaran) ? 'selected' : '';
				echo '>'.$thajaran.'</option>';
			}
			?>
			</select>
		</div>
